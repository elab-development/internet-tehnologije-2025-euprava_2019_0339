...
import { FiCheckCircle, FiArrowRight, FiRefreshCw, FiFileText } from "react-icons/fi";
...
export default function OfficerApprovals() {
  ...
  const load = async () => {
    ...
    setError("");
    try {
      // backend: GET /api/service-requests?status=APPROVED
      const res = await api.get("/api/service-requests?status=APPROVED");
      setItems(res?.data?.data || []);
    } catch (e) {
      setError(e?.response?.data?.message || "Greška pri dohvatanju odobrenih zahteva.");
    } finally {
      setBusy(false);
    }
  };
    ...
    <div className="home-page">
      ...
      <div className="home-wrap">
        <div className="page-head">
          <div>
            <div className="page-title">Odobreni zahtevi</div>
            <div className="page-subtitle">Lista zahteva sa statusom APPROVED.</div>
          </div>
        ...
        {!busy && items.length === 0 ? <div className="eu-empty">Nema odobrenih zahteva.</div> : null}
                ...
                <div className="eu-rowLeft">
                  ...
                  <div className="eu-rowMain">
                    <div className="eu-rowTitle">
                      Zahtev #{x.id} <span className="eu-badge eu-badge--ok">{x.status}</span>
                    </div>
